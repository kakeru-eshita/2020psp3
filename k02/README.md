...
## ソースコードの説明
79.先頭が空のアドレスを指すようにする。
80.新しい要素pNodeのデータ領域を確保するメモリを確保する。
81.pNodeの中のcityにnewCityのデータを入れる。
82.pNodeの中のpNextにpNextのデータを入れる。
83.topが新しいデータの先頭を指すようにする。
84.新しいデータの先頭のアドレスを指すtopの値を返す。

108.検索場所を指定する変数と,検索結果を返すための変数として　pos, resultを宣言する。
109.結果を見つからなかったにしておく。
110.ファイルのサイズ未満までposを１ずつずらす。
111.検索するIDとpListの中のcityのIDが一致した場合を真としたif文。
112.一致したIDを入れる。
113.pCityの中身を検索が一致したcityにする。
114.ループを抜ける。
116.pListの中の次のデータのアドレスを指すようにする。
118.結果を返す。
...
```
PS C:\Users\bb35319010\Desktop\2020psp3\k02> ./k02
