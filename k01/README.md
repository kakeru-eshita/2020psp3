...
## ソースコードの説明
68.比較対象と結果を入れる関数を宣言する。
69.比較する値がデータ数を越えるまで比較数を１増やしながら繰り返す。
70.探すIDと比較対象の位置のIDを比較する。
71.探すIDと比較対象の位置のIDが一致していたらresultにその値を入れる。
72.繰り返しを抜ける。
75.resultの値を返す。

81.比較対象と結果を入れる関数を宣言する。
82.結果を(result)を見つからなかったにしておく。
83.84.リストの左端と右端をそれぞれ0とリストの最後にする
85.リストの左側がリストの右側を越えるまで繰り返す。
86.比較対象の位置をリストの真ん中にする。
87.探すIDと比較対象の位置のIDを比較する。
88.探すIDと比較対象の位置のIDが一致していたらresultにその値を入れる。
89.繰り返しを抜ける。
90.91.探す値の方が大きければリストの左端を比較対象の位置の一つ右にずらす。
92.93.探す値の方が小さければリストの右端を比較対象の位置の一つ左にずらす。
96.resultの値を返す。
## 出力結果

```
PS C:\Users\bb35319010\Desktop\2020psp3\k01> ./k01
